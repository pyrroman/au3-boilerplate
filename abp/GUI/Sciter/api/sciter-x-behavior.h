...
// parameters of evtg == HANDLE_SCROLL
  ...
  /*
  struct RICHTEXT_ENUM_PARAMS; 

  // richtext bookmark - identifies uniquely position inside richtext
  struct RT_BOOKMARK
  {
    UINT p1;
    UINT p2;
  ...
  typedef BOOL CALLBACK RichtextEnumerationCallback( SCROLLBAR_RICHTEXT_ENUM_PARAMS* pp, RT_BOOKMARK pos, RT_POSITION postype, WCHAR code );

  struct RICHTEXT_ENUM_PARAMS: METHOD_PARAMS 
  {
     UINT        what; // 0 - whole document, 1 - selection, 2 - current paragraph, 3 - start/end below:
                       // if OR'ed with 0x80000000 - enumeration is backward, otherwise forward.
     RT_BOOKMARK start;
     RT_BOOKMARK end;
     RichtextEnumerationCallback *pcallback;
  };
  */
