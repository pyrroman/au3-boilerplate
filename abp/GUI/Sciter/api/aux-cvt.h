...
//#pragma warning( suppress:4127 ) //  warning C4127: conditional expression is constant
  ...
  inline unsigned int getc_utf16(const wchar_t* &start, size_t& length)
  {
      if( length == 0 )
        return 0;
      wchar_t c = *start; ++start; --length;
  ...
  inline bool fromwcs(const wchar_t* wcs, size_t length, pod::byte_buffer& outbuf)
  {
    ...
    unsigned int  num_errors = 0;
    unsigned int  c;
    while((c = getc_utf16(wcs,length)) != 0) 
    {
      if (c < (1 << 7)) 
      {
        outbuf.push(byte(c));
  ...
  public:
    ...
    ostream_t& operator << (const wchar_t* wstr)
    {
      ...
      unsigned int c;
      while((c = getc_utf16(wstr,length)) != 0) 
      {
        if(X)
          switch(c) 
          {
...
/** string to integer parser.
  **/
  ...
  public:
    ...
    // use this for text output (will be markup-escaped)
