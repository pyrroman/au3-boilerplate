...
Func _Bass_PluginGetInfo($handle)
	...
	local $BASSArrOfBASS_PLUGINFORM = DllStructCreate($ArrOfBASS_PLUGINFORM_TAG, DllStructGetData($bp, "formats"))
...
; ===============================================================================================================================
Func _BASS_Apply3D()
	Local $BASS_ret_ = DllCall($_ghBassDll, "none", "BASS_Apply3D")
...
; Example .......;
; ===============================================================================================================================
Func _BASS_MusicLoad($mem, $file, $offset, $offsethigh, $length, $flags, $freq)
	Local $tpFile="ptr"
...
; Example .......;
; ===============================================================================================================================
Func _BASS_SampleLoad($mem, $file, $offset, $offsethigh, $length, $max, $flags)
	Local $tpFile = "ptr"
	If IsString($file) Then $tpFile = "wstr"
...
Func _BASS_SampleGetInfo($handle)
	Local $reta[17]
	local $BASS_SAMPLE_INFO_S = DllStructCreate($BASS_SAMPLE)
...
Func _BASS_StreamCreateURL($url, $offset, $flags, $proc = 0, $user = 0)
	Local $dcProc=-1
	If IsString($proc) Then
		Local $dcProc = DllCallbackRegister ($proc, "ptr", "ptr;dword;ptr;")
...
Func _BASS_RecordGetInputName($inputn)
    ...
    local $struc = DllStructCreate ("char[255]", $BASS_ret_[0])
...
Func _BASS_ChannelGetInfo($handle)
	Local $aRet[8]
	local $dsInfo = DllStructCreate($BASS_CHANNELINFO)
...
Func _BASS_ChannelSetFX($handle, $type, $priority)
	...
	Assign("_gtBASS_FX_" & $BASS_ret_[0], Eval(StringReplace($type, "_FX", "")), 2)
	Return $BASS_ret_[0]
...
Func _BASS_FXSetParameters($fxhandle, $parameters)
	...
	local $struct = DllStructCreate(Eval("_gtBASS_FX_" & $fxhandle))
	For $i = 1 To $param[0]
...
Func _BASS_FXGetParameters($fxhandle)
	Local $sRet = DllStructCreate(Eval("_gtBASS_FX_" & $fxhandle))
