...
struct tabs: public event_handler, behavior_factory
{
    ...
    virtual bool subscription( HELEMENT he, UINT& event_groups )
    {
      event_groups = HANDLE_MOUSE | 
                     ...
                     HANDLE_SCRIPTING_METHOD_CALL // see handle_scripting_call below. 
                     ;
      return true;
    }
   ...
   // methods for scripting:
    bool handle_scripting_call(HELEMENT he, SCRIPTING_METHOD_PARAMS& params ) 
    { 
        #define FUNCTION(ARGC, NAME) if( params.argc == ARGC && aux::streqi(params.name,#NAME) )

        FUNCTION( 1, selectTab )
        {
          json::string tab_name = params.argv[0].get(L"");
          dom::element tab_el = dom::element(he).find_first(":root>.strip>[panel=%S]",tab_name.c_str());
          if(tab_el.is_valid()) {
            bool r = select_tab(dom::element(he),tab_el);
            params.result = json::value(r);
          }
          return true;
        }
        return false;
